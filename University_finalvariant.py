...
def clear_db():
    try:
        ...
        delete_table = "DROP TABLE Teachers"
        cursor.execute(delete_table)
        delete_table = "DROP TABLE Courses"
        cursor.execute(delete_table)
        delete_table = "DROP TABLE Exams"
        cursor.execute(delete_table)
        delete_table = "DROP TABLE Grades"
        cursor.execute(delete_table)
...
def InputDateToUpdate():
    date = input("Введите дату в формате 'YYYY-MM-DD' или ничего не вводите если не хотите менять: ")
    if len(date) == 10 and (date[:4]).isdigit() and (date[5:7]).isdigit() and (date[8:]).isdigit() and date[4] == '-' and date[7] == '-':
            return date
    if date ==" " or date =="  ":
        return date
...
def InputCourseId():
    id = input("Введите ID курса: ")
    if id.isdigit() and int(id) > Count_Of_Lines("Courses"):
...
def InputStudentId():
    ...
    if id.isdigit() and int(id) > Count_Of_Lines("Students"):
...
def InputTeacherId():
    id = input("Введите ID Преподавателя: ")
    if id.isdigit() and int(id) > Count_Of_Lines("Teachers"):
...
def InputExamId():
    id = input("Введите ID экзамена: ")
    if id.isdigit() and int(id) > Count_Of_Lines("Exams"):
...
def InputMaxScore():
    while True:
        try:
            max_score = int(input("Введите максимальный балл за экзамен (от 100 до 105): "))
            if 100<= max_score <= 105:
                return max_score
            else:
                print("Оценка должна быть в диапазоне от 100 до 105.")
        except ValueError:
            print("Введите целое число.")
def ADD_New_Student():
...
def Update_Student():
    ...
    new_date_of_birth = InputDateToUpdate()
