...
def InputDate():
    date = input("Введите дату рождения в формате 'YYYY-MM-DD': ")
...
def ADD_New_Exam():
    ...
    exams_to_insert = [(InputDate(),
                        InputId(),
                        int(input("Введите максимальный балл экзамена: ")+ InputScore()))]
    try:
...
def Get_AVG_Grade():
    ...
    try:
        ...
        result = cursor.fetchone()
        if result[0] is not None:
            whole_average = result[0]
        else:
            a = 0
        print(f"Средний балл студента с ID {student_id} в целом: {whole_average}")
    except sqlite3.Error as err:
        print(f"Ошибка при получении среднего балла: {err}")
...
def Get_AVG_Department_Grade():
    ...
    try:
        ...
        result = cursor.fetchone()
        if result[0] is not None:
            department_average = result[0] 
        else:
            a = 0
        print(f"Средний балл в целом по факультету '{department}': {department_average}")
    except sqlite3.Error as err:
        print(f"Ошибка при получении среднего балла: {err}")
